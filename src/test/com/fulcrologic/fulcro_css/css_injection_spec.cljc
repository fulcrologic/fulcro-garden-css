(ns com.fulcrologic.fulcro-css.css-injection-spec
  (:require
    [fulcro-spec.core :refer [specification assertions behavior when-mocking]]
    [com.fulcrologic.fulcro-css.css-injection :as injection]
...
(defsc DuplicateCSS [_ _]
  {:query [{:a (comp/get-query A)} {:b (comp/get-query A)}]})

(defsc ASibling [_ _]
  {:query [{:x (comp/get-query A)}]})

(defsc BSibling [_ _]
  {:query [{:x (comp/get-query B)}]})

(defsc NestedRootWithCSS [_ _]
  {:css   []
   :query [{:a (comp/get-query ASibling)} {:b (comp/get-query BSibling)}]})

(defsc NestedRoot2 [_ _]
  {:css         []
   :css-include [A]
   :query       [{:a (comp/get-query A)} {:b (comp/get-query BSibling)}]})
